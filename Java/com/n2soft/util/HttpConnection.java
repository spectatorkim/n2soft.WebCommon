...
public class HttpConnection {
	...
	public HttpConnection(String spec, boolean isMulti) throws Exception {
		set(spec, isMulti);
	}

	public HttpConnection(String spec) throws Exception {
		set(spec);
	}
	...
	public void toFile(File f) throws Exception {
		toFile(new FileOutputStream(f));
	}

	public void toFile(OutputStream os) throws Exception {
		if (spec.contains("?"))
			url = new URL(spec + "&" + toString());
		else
			url = new URL(spec + "?" + toString());
		...
		for(int i=0; (read_len = bis.read(buffer)) != -1; i++) {
			os.write(buffer, 0, read_len);
			total_len += read_len;
			if( i > 0 && i % 100 == 0 ) {
				System.out.println(UtilMgr.commaFormat(total_len+""));
				os.flush();
			}
		}
