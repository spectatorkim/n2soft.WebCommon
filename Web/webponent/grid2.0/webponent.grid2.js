...
/*		if (new Date() > new Date(licenseObject.expireDate * 1)) {
	...
	(function () {
		...
		grid.settings = cloneSettingModel(); initialzeSettingModel(grid.settings); grid.markup = cloneMarkupModel(); grid.settings.option = _.extend(grid.settings.option, option); grid.settings.table.markup = table; if (!template.is(table)) {
			...
			} var settingHeight = parseInt(grid.settings.template.markup.attr("height"), 10); if (_.isNaN(settingHeight)) {
				settingHeight = "auto";
		...
		function calculateFixedFreezeScrollerInnerWidth(grid) {
			var width = 0; var columns = grid.settings.columns; _.each(grid.settings.columns, function (column, index, list) {
				if (column.visibility !== "hidden" && !column.fixed && column.level === 0) {
					...
					if (!_.isUndefined(column.displayingWidthTmp)) {
						width += parseInt(column.displayingWidthTmp);
					}
					else {
						width += widthSum;
					}
				}
			}); console.log("%%%%%%%width%%%%%%%" + width); var columnCount = 0; for (var i = columns.length - 1; i >= 0; i--) {
		...
		function setColumnVisibilityWhenFixedLayoutScrolling(grid) {
			console.log("----------------------------------------------"); var maxLevel = grid.settings.headerMaxLevel; var filteredColumns = _.filter(grid.settings.columns, function (column) {
				var f = false; if (column.fixed === false && column.level === maxLevel) {
					f = true;
				} if (column.fixed === false && (column.level + column.rowspan - 1) === maxLevel) {
					f = true;
				} return f;
			}); var columns = []; var bodyHeaderCells = $(grid.markup.main.body.thead).find("th, td"); var fixedColumnCount = getFixedColumnCount(grid); bodyHeaderCells.each(function (i) {
			...
			}); console.log(">> " + grid.markup.functional.freezeScroller.scrollLeft); var widthSum2 = 0; for (var i = 0, end = columns.length; i < end; i++) {
				try {
					if (!_.isUndefined(columns[i].displayingWidthTmp) && (columns[i].displayingWidth != columns[i].displayingWidthTmp)) {
						console.log(">>#" + i + " #==############# > " + columns[i].displayingWidth + " <<====(set) " + columns[i].displayingWidthTmp);
						columns[i].displayingWidth = columns[i].displayingWidthTmp;
						columns[i].width = parseInt(columns[i].displayingWidthTmp);
						columns[i].displayingWidthTmp = undefined;
						var parentColumn = columns[i].parentColumn;
						if (parentColumn !== null) {
							setParentColumnWidthAndColspan(parentColumn, false);
							console.log("XXXXXXXXXXXXXXX");
						}
					}
				}
				catch (e) { }
				widthSum2 = widthSum2 + columns[i].width;
				console.log(">> " + i + " #==############# > width:" + columns[i].width + "=== " + widthSum2);
			} var scrollLeft = grid.markup.functional.freezeScroller.scrollLeft; var widthSum = 0; var newViewIndex = null; var ignoreCount = grid.settings.option.fixedScrollAdjustment; for (var i = 0, end = columns.length - ignoreCount; i < end; i++) {
				if (columns[i].visibility !== "hidden") {
				...
				for (var i = columns.length - 1; i >= 0; i--) {
					...
					console.log(i + "   check w:" + w + " sum:" + leftWidthSum + " tmp:" + columns[i].displayingWidthTmp + " width:" + columns[i].width + "  " + columns.length);
				...
				console.log("*************************************************");
				console.log("*************************************************");
				console.log("grid.markup.functional.freezeScroller.style.width===" + grid.markup.functional.freezeScroller.style.width);
				console.log("scrollBodyWidth======" + scrollBodyWidth);
				console.log("lastViewIndex ======" + grid.lastViewIndex);
				console.log("firstViewColumnWidth======" + grid.firstViewColumnWidth);
			} if (newViewIndex > grid.lastViewIndex) {
				newViewIndex = grid.lastViewIndex;
			} var currentViewIndex = grid.settings.freezeScroll.currentViewIndex; console.log("newViewIndex ======" + newViewIndex + "  scrollLeft:" + scrollLeft + " widthSum:" + widthSum + " " + currentViewIndex + " *" + grid.lastViewIndex); if (newViewIndex !== null) {
				var hideStart = 0;
				...
				for (var j = hideStart, jEnd = columns.length; j < jEnd; j++) {
					...
					console.log(j + " | " + grid.lastViewIndex + " | " + j + " | " + hideEnd);
					if (j == grid.lastViewIndex && j == hideEnd) {
						...
						console.log(">>grid. tmp 설정 ====== " + j + " " + targetColumn.displayingWidth + " " + targetColumn.displayingWidthTmp);
					}
		...
		function renderLayout(grid) {
			var table = grid.settings.table.markup; var gridArea = $(grid.markup.area); if (grid.settings.option.id !== null) {
			...
			} var wrapper = $(grid.markup.wrapper); wrapper.css({ "padding-right": getScrollBarWidth(grid) }); table.before(gridArea); gridArea.append(wrapper); table.remove(); var mainWrapper = $(grid.markup.main.wrapper); mainWrapper.css({ overflow: "hidden", position: "relative", "z-index": 3 }); var headerWrapper = $(grid.markup.main.header.wrapper); var headerScroller = $(grid.markup.main.header.scroller); headerScroller.css({ overflow: "hidden" }); var headerInner = $(grid.markup.main.header.inner); headerInner.css({ position: "relative" }); var headerTable = $(grid.markup.main.header.table); var bodyWrapper = $(grid.markup.main.body.wrapper); var bodyScroller = $(grid.markup.main.body.scroller); bodyScroller.css({ "overflow-y": "hidden", "overflow-x": "hidden", position: "relative" }); var bodyInner = $(grid.markup.main.body.inner); bodyInner.css({ position: "relative", "min-height": "1px" }); var bodyTable = $(grid.markup.main.body.table); bodyTable.css("z-index", "1"); headerInner.append(headerTable); headerScroller.append(headerInner); headerWrapper.append(headerScroller); mainWrapper.append(headerWrapper); bodyTable.prepend(grid.markup.main.body.caption); bodyTable.append(grid.markup.main.body.tbody); bodyInner.append(bodyTable); bodyScroller.append(bodyInner); bodyWrapper.append(bodyScroller); mainWrapper.append(bodyWrapper); wrapper.append(mainWrapper); var freezeScroller = $(grid.markup.functional.freezeScroller); freezeScroller.css({ position: "absolute", top: 0, left: 0, "z-index": 2, "overflow-y": "scroll" }); if (navigator.appVersion.indexOf("MSIE 9.") !== -1) {
				freezeScroller.css({ "padding-right": getScrollBarWidth(grid), "padding-bottom": getScrollBarWidth(grid) });
			} var freezeScrollerInner = $(grid.markup.functional.freezeScrollerInner); freezeScrollerInner.css({ "min-height": "1px" }); freezeScroller.append(freezeScrollerInner); var freezeIndicatorLeft = $(grid.markup.functional.freezeIndicatorLeft); var freezeIndicatorRight = $(grid.markup.functional.freezeIndicatorRight); gridArea.append(freezeIndicatorLeft); gridArea.append(freezeIndicatorRight); wrapper.append(freezeScroller); if (getScrollBarWidth(grid) > 0) {
			...
			} renderHeader(grid); setRowTemplates(grid); var bodyThead = $(grid.markup.main.body.thead); var mainRow = $(grid.settings.template.mainRow); mainRow.filter("tr").each(function (i) { var mainRowTr = $(this); var bodyTheadRow = bodyThead.find("tr").eq(i); mainRowTr.children().each(function () { var cell = $(this); var cellName = cell.attr("name"); var bodyTheadRowCell = getjQueryElementByName(bodyTheadRow, cellName); bodyTheadRow.append(bodyTheadRowCell); }); }); if (grid.settings.option.paging !== undefined) {
				setPageLayout(grid);
			}
			else {
		...
		function bindMarkupEvents(grid) {
			if (grid.settings.option.autoResize !== false) {
				...
				if (e.deltaX === 0) {
					...
					var status = "in-grid";
					if (grid.markup.functional.freezeScrollerInner.clientHeight < grid.markup.functional.freezeScroller.clientHeight) {
						status = "bottom-of-grid";
					}
					if (scrollTop === 0 && e.deltaY > 0) {
						status = "top-of-grid";
					}
					if (scrollTop === freezeScroller.data("prev-top") && e.deltaY < 0) {
						status = "bottom-of-grid";
					}
					if (grid.settings.usePaging) {
						status = "bottom-of-grid";
					...
					if (grid.settings.option.preventDefaultScroll) {
						e.preventDefault();
					}
					if (scrollTop === 0) {
						scrollTop = null;
				...
				mainBodyTbody.on("mousedown", "tr", function (e) {
					if (e.shiftKey) {
						...
						if (e.shiftKey) {
							...
							$(gridRows).removeClass(GRID_CLASS.FUNCTIONAL.ROW_SELECTED);
							var whereRows = _.where(rowModels, { selected: true });
							$.each(whereRows, function (index, value) { value.selected = false; });
							if (!isSelected) {
								clickTr.addClass(GRID_CLASS.FUNCTIONAL.ROW_SELECTED);
								rowModels[clickIdx].selected = true;
							}
						}
					} if (!e.shiftKey) {
		...
		function renderColumnWidth(grid, outerWidth, parentColumn) {
			var columns = null; var parentWidth = null; if (!parentColumn) {
			...
			} var allAuto = true; _.each(columns, function (column, index, list) {
				if (column.displayingWidth !== "*") {
					allAuto = false;
				}
			}); var allPercent = true; _.each(columns, function (column, index, list) {
				...
				var width = column.width; var header = $(column.header); var copiedHeader = $(column.copiedHeader); copiedHeader.attr("colspan", column.colspan); if (column.visibility === "hidden" || column.visibility === "temporarilyHidden") {
					...
					var checkPaddingLeft = parseFloat(header.css("padding-left"));
					var checkPaddingRight = parseFloat(header.css("padding-right"));
					var checkBorderRight = parseFloat(header.css("border-right"));
					var checkBorderLeft = parseFloat(header.css("border-left"));
					var paddingWithBorder = checkPaddingLeft + checkPaddingRight + checkBorderRight + checkBorderLeft;
		...
		function getTrHeight(grid) {
			var checkedHeight = grid.settings.trHeight; if (checkedHeight && checkedHeight > 0) {
				...
				flag = true;
			} var trHeight = firstTr.height(); if (flag) {
				firstTr.remove();
			} grid.settings.trHeight = trHeight; return trHeight;
		}
		...
		function makeSortingUI(grid) {
			var columns = grid.settings.columns; _.each(columns, function (column, index, list) {
				if (column.childColumns.length === 0 && column.name !== "CI_GRID_CHECKBOX_CONTAINER" && column.colspan === 1 && column.sortable) {
		...
		function setChildColumnVisibility(column, visibilityString) {
			var childColumns = column.childColumns; _.each(childColumns, function (childColumn) {
		...
		grid.lastViewIndex = -1; grid.markup.functional.freezeScroller.scrollLeft = 0; renderGridHeightAndWidth(grid); reLayoutHeader(grid); if (grid.settings.option.niceScroll) {
		...
		}; grid.destroyFixedColumn = function () { destroyFixedLayout(grid); return "Fixed columns are destroyed successfully."; }; grid.appendRow = function (row, index) { addRows(grid, row, "append", index); }; grid.prependRow = function (row, index) { addRows(grid, row, "prepend", index); }; grid.removeRow = function (rowOrStart, end) {
			if (rowOrStart === false) {
				clearRows(grid);
		...
		function bindEvents(grid) {
			...
			grid.lastViewIndex = -1; renderColumnResizer(grid); setColumnVisibilityWhenFixedLayoutScrolling(grid); renderGridHeightAndWidth(grid); if (grid.settings.option.niceScroll) {
				$(grid.markup.functional.freezeScroller).getNiceScroll().resize();
			}
